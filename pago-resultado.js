(() => {
  ...
  async function init() {
    ...
    if (!txId) {
      showError(
        "Sin transaccion",
        "No encontramos el ID de pago en la URL. Si ya pagaste, revisa tu correo o escribenos por WhatsApp."
      );
      return;
    }

    try {
      const tx = await fetchWompiTx(txId);
      if (!tx) {
      ...
      const code = order?.orderCode || tx.reference || "—";
      ...
      if (status === "APPROVED") {
        els.heroTitle.textContent = "¡Gracias por tu compra!";
        els.heroLead.textContent =
          "Tu pedido ha sido recibido correctamente. Te enviaremos un correo con los detalles de tu compra.";
        els.timeline.querySelector('[data-step="1"]')?.classList.add("is-done");
      } else if (status === "PENDING" || status === "PENDING_PAYMENT") {
        els.heroTitle.textContent = "Pago en proceso";
        els.heroLead.textContent =
          "Tu pago aun no esta confirmado. Si ya completaste el pago, espera unos minutos y recarga esta pagina.";
      } else {
        els.heroTitle.textContent = "Pago no completado";
        els.heroLead.textContent =
          "La transaccion no pudo completarse. Puedes volver al catalogo e intentar de nuevo.";
      }
